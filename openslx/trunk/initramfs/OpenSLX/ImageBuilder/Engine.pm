...
# -----------------------------------------------------------------------------
# ImageBuilder::MakeInitialRamFS::Engine.pm
#    - provides driver engine for MakeInitialRamFS API.
# -----------------------------------------------------------------------------
package OpenSLX::ImageBuilder::MakeInitRamFS::Engine;
...
sub new
{
    ...
    $self->{distro} = loadDistroModule({
        ...
        distroScope => 'OpenSLX::ImageBuilder::Distro',
    });
    if (!$self->{distro}) {
        die _tr(
            'unable to load any ImageBuilder::Distro module for system %s!',
            $self->{'system-name'}
        );
    }
...
sub _copyKernelModules
{
    ...
    # TODO: find out what's the story behind the supposedly required
...
OpenSLX::ImageBuilder::MakeInitRamFS::Engine
