...
elseif ( $automove && $hostsmove ){
	...
	foreach ($hostsmove as $hostname){
		...
		if (!$found_hostdn || $found_hostdn == $selfhost) {
			...
			/*if ($add = ldap_add($ds,$newhostdn,$newhost)) {
				if ($delete = delete_host($selfhost)) {
					if ($newhost['dhcphlpcont']) {
						$dhcpchange = 1;
					}
					$mesg .= "Client <b>$hostname</b> erfolgreich nach <b>$target_ou[0] ($target_zone)</b> verschoben<br><br>";
				}else{
					$mesg .= "Fehler beim l&ouml;schen von Client <b>$hostname</b> aus <b>$au_ou ($assocdom)</b><br>";
					if ($delete2 = delete_host($newhostdn)) {
						$mesg .= "Kopierter Client <b>$hostname</b> wieder aus <b>$target_ou[0]</b> entfernt";
					}else{
						$mesg .= "Fehler beim wieder entfernen von <b>$hostname</b> aus <b>$target_ou[0]</b>. Client jetzt in beiden AUs angelegt<br>";
					}
				}
			}else{
				$mesg .= "Fehler beim hinzuf&uuml;gen von Client <b>$hostname</b> in <b>$target_ou[0] ($target_zone)</b><br>";		
			}*/
