...
function merge_2_ipranges($iprange1,$iprange2)
{
	...
	$oct1 = explode('.',$ipr1exploded[0]);
	$oct2 = explode('.',$ipr2exploded[0]);	
	if ($oct1[0] == $oct2[0] && $oct1[1] == $oct2[1] && $oct1[2] == $oct2[2]) {	
	...
	if ( ($ipr1e + 1) >= $ipr2s && $ipr1s <= ($ipr2e + 1) ){
		...
		# printf("No Merging possible: %s and %s<br>Not same Subnet",$iprange1,$iprange2);
		return 0;
	}
}
...
* @author Tarik Gasmi
*/
function get_freeipblocks_au($au)
{
	global $ds, $suffix, $ldapError;

	if(!($result = uniLdapSearch($ds, $au, "objectclass=*", array("FreeIPBlock"), "", "one", 0, 0))) {
...
* @author Tarik Gasmi
*/
function get_maxipblocks_au($au)
{
	global $ds, $suffix, $ldapError;

	if(!($result = uniLdapSearch($ds, $au, "objectclass=*", array("MaxIPBlock"), "", "one", 0, 0))) {
      # redirect(5, "", $ldapError, FALSE);
      echo "search problem";
      die;
   } else {
...
function get_maxipblocks_au_childs($au)
{
	global $ds, $suffix, $ldapError;

	if(!($result = uniLdapSearch($ds, $au, "objectclass=administrativeUnit", array("MaxIPBlock","ou"), "", "list", 0, 0))) {
      # redirect(5, "", $ldapError, FALSE);
      echo "search problem";
      die;
   } else {
   ...
   #echo "<br>FIPB: <br>";print_r($fipb_array);echo "<br>";
	...
	if ( $ipmatch ){	
		...
		if ($results){
	   	...
	   	echo "Fehler beim eintragen der FIPBs!<br>";
	   	return 0;
	   }	      
	}
	else{
		printf("IP Adresse %s nicht im verfuegbaren Bereich!<br>", $ip );
		return 0;
	}
}
...
function add_dhcprange($newrange,$pooldn) {
   ...
   $ipmatch = 0;
   $fipb_array = get_freeipblocks_au($poolaudn);
   #echo "<br>FIPB: <br>";print_r($fipb_array);echo "<br>";
	...
	if ( $ipmatch ) {
		...
		$result1 = ldap_mod_replace($ds,$poolaudn,$entry);
		if ($result1){
			printf("<br>Neue FIPBs erfolgreich eingetragen!<br>");
			...
			if ($result2){
	   		...
	   		ldap_mod_add($ds,$poolaudn,$entry2);
	   		merge_ipranges($poolaudn);
	   		return 0;
	   	}
		}else{
	   	printf("<br>Fehler beim eintragen der FIPBs!<br>");
	   	return 0;
	   }	  		
	}else{
...
* @author Tarik Gasmi
*/
function delete_ip_dhcprange($dhcpobjectDN,$au_dn)
{
	global $ds, $suffix, $ldapError;
	
	# oder $au_dn aus dhcpobjectdn generieren ... 
...
function delete_dhcpranges($oldranges_array,$dhcpobjectDN) {
   
   global $ds, $auDN, $suffix, $ldapError;
   
   foreach ($oldranges_array as $oldrange) {
   	
   	$addentry ['freeipblock'] = $oldrange;
   	$delentry ['dhcpRange'] = $oldrange;
   	$result1 = ldap_mod_del($ds,$pooldn,$delentry);
   	if ($result1) {
   		$result2 = ldap_mod_add($ds,$auDN,$addentry);
   		if ($result2) {
   			merge_ipranges($auDN);
   			echo "<br>IP Range $oldrange erfolgreich aus Pool gel&ouml;scht!
   						<br>und erfolgreich als neuer FIPB in die AU eingetragen!<br>";
   			return 1;
   		}else{
   			echo "Fehler beim eintragen als neuer FIPB!";
   			ldap_mod_add($ds,$pooldn,$delentry);
   			return 0;
   		}
  		}else{
   		echo "Fehler beim l&ouml;schen der Pool IP Range $oldrange1[$i] - $oldrange2[$i]";
  			return 0;
   	}
	}   
}
...
function objects_to_delete($delip,$childauDN,$cchild_array)
{
   ...
   $objects_to_delete = array('hostips' => $chosts_todo,
   									'subnetips' => $csubnets_todo,
   									'dhcpranges' => $cpools_todo);
