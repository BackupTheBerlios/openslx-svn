...
$oldchilddomain = strtolower($_POST['oldchilddomain']);
...
if ($oldchilddomain != $childdomain) {
	...
	if ( $childdomain == "" ) {
		echo "Domain l&ouml;schen bzw. integrieren<br><br>";
		$delmodus = "integrate";
		delete_child_domain($oldchilddomain,$assocdom,$childDN, $domDN, $delmodus);
		$seconds = 5;
	}
	elseif ( $oldchilddomain == "" ) {
		echo "Domain eintragen <br><br>";
		$seconds = 5;
	}
	else{
		...
		if (change_child_domain($childdomain, $oldchilddomain, $childDN, $assocdom, $domDN, $domprefix)) {
			$mesg = "Domain erfolgreich ge&auml;ndert<br><br>";
		}
		else {
			$mesg = "Fehler beim &auml;ndern der Domain<br><br>";
		}
	}
